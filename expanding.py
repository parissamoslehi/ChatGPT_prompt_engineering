...
def customize_the_automated_reply_to_a_customer_email(review, sentiment):
    prompt = f"""
...
def use_details_from_the_customers_email(review, sentiment):
    prompt = f"""
    You are a customer service AI assistant.
    Your task is to send an email reply to a valued customer.
    Given the customer email delimited by ```, \
...
def run():
    ...
    two days.
    """
    customize_the_automated_reply_to_a_customer_email(review, sentiment)
    # usage of temperature
    use_details_from_the_customers_email(review, sentiment)
